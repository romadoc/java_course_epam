...
public class Main16 {
    public static void main(String[] args) {
      ...
      List<Integer>list = unevenList(array);
      int sum = summatorUnevenDigits(list);
    ...
    private static List<Integer> unevenList(int[]arrayIn){
        List<Integer>list = new ArrayList<>();
        for (int i = 0; i < arrayIn.length; i++ ){
            List<Integer> subList = arrayConstructor(arrayIn[i]);
            int count = 0;
            int sum = 0;
    ...
    private static int summatorUnevenDigits(List<Integer>listIn){
        int sum = 0;
    ...
    private static void evenFinder(int sum){
        int evenCuantaty = 0;
        int x = sum;
        int count = 0;
        List<Integer>list = new ArrayList<>();
